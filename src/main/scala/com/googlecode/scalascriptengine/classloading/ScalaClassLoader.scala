...
case class ScalaClassLoader(
  ...
  /**
	 ...
	 */
  def all = if (config.enableClassRegistry) allClasses
  else throw new IllegalStateException("ClassLoaderConfig.enableClassRegistry not true, class registry not enabled")

  def withTypeOf[T](implicit ct: ClassTag[T]): List[Class[T]] =
    all.filter(ct.runtimeClass.isAssignableFrom _).asInstanceOf[List[Class[T]]]
  ...
  override def loadClass(name: String) = {
    ...
    config.classLoadingListeners.foreach { cll =>
      cll(name, clz)
    }
    clz
  }
